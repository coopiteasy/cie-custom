...
class SaleOrder(models.Model):
    ...
    def _cart_update(
        ...
        lines_to_remove = self.order_line.filtered(lambda line: line.product_id.is_container)
        containers_to_add = {}  # container: amount

        for line in lines_to_remove:
            self._cart_update(line.product_id.id, line.id, add_qty=0, set_qty=-1, **kwargs)
